...
import com.portal.app.service.PropertyFloorService;
...
@RestController
@RequestMapping("/api/property/floor")
public class PropertyFloorController {

    private static final Logger logger = LoggerFactory.getLogger(PropertyFloorController.class);

    @Autowired
    private PropertyFloorService floorService;
    ...
    public ResponseEntity<?> get(@PathVariable(value = "propId") Long propId) {
        LoggerUtil.debug(logger, "IN :: get :: floor plan :: propId : ", propId);
        PropertyFloor response = floorService.getFloorPlanByPropId(propId);
        if (response == null) {
            logger.error("No data returned from floorService");
            LoggerUtil.debug(logger, "OUT :: get :: floor plan", StringUtils.EMPTY);
    ...
    public ResponseEntity<?> processFloorInfo(@CurrentUser UserPrincipal currentUser, @Valid @ModelAttribute FloorRequest request) {
        LoggerUtil.debug(logger, "IN :: processFloorInfo :: request details : ", request);
        try {
            PropertyFloor response = propertyService.savePropertyFloorInfo(request, currentUser.getId(), request.getpId());
            if (response == null) {
                logger.error("No data returned from floorService");
