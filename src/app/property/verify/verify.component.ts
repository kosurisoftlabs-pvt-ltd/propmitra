...
export class VerifyComponent implements OnInit {
  ...
  public verify(): void {
    const selectedProps = this.verifyForm2.value.propChks
    ...
    if (this.isPropSelected && this.verifyForm2.valid) {
      let formData: FormData = new FormData();
      formData.append("value", "1");
      formData.append("propIds", selectedProps);
      formData.append("comments", this.verifyForm2.value.comments);
      this.spinner.show();
      this.pService.verifyProprties(formData)
        .subscribe(
          res => {
            //console.log("res", res);
            this.spinner.hide();
            if (res.status == 200) {
              ...
              setTimeout(() => this.toastr.success('Selected properties verified successfully.', 'Success!', {
                timeOut: 4000
              }));
            }
          },
      ...
      Object.keys(this.verifyForm2.controls).forEach(field => { // {1}
        const control = this.verifyForm2.get(field);            // {2}
  ...
  private addCheckboxes() {
    this.propList.map((o, i) => {
      ...
      (this.verifyForm2.controls.propChks as FormArray).push(control);
    });
  }
  ...
  openDialog(id: number, isActive: number): void {
    ...
    if (isActive == 1) {
      ...
      msg = 'Do you confirm the unverification of this property?';
      active = 0;
    } else if (isActive == 0) {
      title = 'Verify Property';
      msg = 'Do you confirm the verification of this property?';
      active = 1;
    }
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.spinner.show();
        this.statusService.toggleStatus(id, active, 'verify')
          .subscribe(
            res => {
              this.spinner.hide();
              if (res.status == 200) {
